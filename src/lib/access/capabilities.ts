...
export const CAPABILITIES = [
  ...
  {
    key: "feePetition.manage",
    label: "Manage Fee Petitions list",
    description:
      "Add cases to the Fee Petitions page, and remove or clear them from it.",
  },
  {
...
//   leader notes + managing the Fee Petitions list, but NOT create, delete,
//   editing fee amounts received, PIF, or Fees Closed (stays admin-only).
...
//   (close/overpaid/approvedBy), no PII, no leader notes, no PIF, and no
//   adding/removing cases on the Fee Petitions page. Admins can widen any of
//   these per-user via the access overrides modal.
export const ROLE_CAPABILITY_DEFAULTS: Record<Role, CapabilityKey[]> = {
  system_admin: ALL,
  admin: ALL.filter((k) => k !== "fees.edit" && k !== "feesConfirmation.edit"),
  lead: ["case.update", "case.finalize", "case.editPii", "dailyMetrics.editOthers", "leaderNotes.access", "feePetition.manage"],
  member: ["case.update"],
};
