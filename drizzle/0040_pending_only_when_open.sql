-- 0039 accidentally recomputed Pending (Fee Due - Received) unconditionally
-- for every write, including closed records — the Pending auto-calc, like
-- the PIF automation it feeds, is a Master Fees (open-case) concept only.
-- Fees Closed is a finalized historical view; it should never be touched by
-- this trigger going forward. Moves the Pending computation inside the same
-- "NOT NEW.is_closed" guard already used for the PIF classification below.
...
BEGIN
  -- Compute totals (unchanged — these still apply regardless of is_closed,
  -- since Fees Closed's own totals/reporting reads them)
