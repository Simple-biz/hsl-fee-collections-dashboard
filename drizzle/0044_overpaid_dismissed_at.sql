ALTER TABLE "fee_records" ADD COLUMN "overpaid_dismissed_at" timestamp with time zone;

-- Overpaid Cases can now be "removed" (staff dismiss a row without
-- affecting PIF/marked_overpaid, which stay mathematically accurate). That
-- dismissal must not be permanent for a case that later has a genuinely
-- new overpayment — only a *continuing* overpayment (already marked
-- before this write) leaves an existing dismissal alone.
    ...
    IF NOT v_skip THEN
      ...
      IF v_covered AND rel_any_started THEN
        IF rel_any_overpaid THEN
          ...
          -- A genuinely new overpayment (wasn't already marked before this
          -- write) clears any prior dismissal so it surfaces again; a
          -- continuing one (already marked) leaves an existing dismissal
          -- untouched.
          IF TG_OP = 'INSERT' OR NOT COALESCE(OLD.marked_overpaid, false) THEN
            NEW.overpaid_dismissed_at := NULL;
          END IF;
