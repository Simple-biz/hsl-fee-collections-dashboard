-- 0038 dropped the NOT NULL default on t16/t2/aux_fee_due so an untouched
-- section could be told apart from an explicit $0.00, but never guarded the
-- totals arithmetic against it — total_retro_due/total_fees_expected/
-- total_fees_paid, and pif_ready_to_close, all silently went NULL for any
-- case with an untouched Fee Due (confirmed against production: 7 real
-- cases). Wraps every operand in COALESCE so an untouched field contributes
-- $0 to the sums, same as it always has for Retro/Received.
