-- 0041 treated "Fee Due explicitly $0.00" the same as "Fee Due never
-- entered" for the Pending/anomaly check. That's wrong: Overpaid is defined
-- purely as Received > Fee Due (which then makes the auto-calculated
-- Pending column negative) — and that comparison is valid the moment a Fee
-- Due value exists at all, even if it's $0.00. Pending is reserved
-- specifically for when Fee Due was never entered (still NULL/blank), so
-- there's nothing yet to compare Received against.
